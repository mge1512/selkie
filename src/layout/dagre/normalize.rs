...
pub fn assign_node_intersects(graph: &mut DagreGraph) {
    ...
    for (v, w, node_v, node_w, mut points) in edge_data {
        ...
        // The midpoint should use the coordinate of whichever node is FURTHER from
        // graph center. This creates curves that bow toward the outer node.
        if points.len() == 2 {
            let mid_point = if is_horizontal {
                ...
                let mid_y = if src_dist >= tgt_dist {
                    start_point.y
                } else {
                    end_point.y
                };
                ...
                let mid_x = if src_dist >= tgt_dist {
                    start_point.x
                } else {
                    end_point.x
                };
